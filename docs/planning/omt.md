# 한번더(OneMoreTime) — 기능 명세서

2026-09-25 · 프로토타입(`docs/prototype/`) 기준 · 허브 명세는 [planning.md](planning.md)

## 개요

**한번더(OneMoreTime)** 는 주사위 4개로 11개의 갱도를 파내려가, 가장 깊은 곳의 보물 3개를 먼저 찾는 쪽이 이기는 보드게임이다. 게임 방식은 캔트 스톱과 같고, 테마는 산 등반 대신 땅을 파는 던전 탐험이다. 놀이터 허브에 올라가는 카드 중 하나다.

이 문서는 **실제로 동작하는 프로토타입을 만들고 나서 확정된 내용**이다. 화면·버튼·수치는 모두 프로토타입에서 가져왔으므로, 구현 중 이 문서와 프로토타입이 어긋나면 프로토타입이 맞다.

형식은 **기능명 / 목적 / 위치 / 트리거 / 처리 로직** 중심이고, 번호(`1-1` ~ `5-3`)는 프로토타입 HTML의 `data-spec` 속성 및 [ERD](../erd/erd.md)·[API 명세](../api/api.md)와 1:1로 대응한다.

> 화면 전환·토스트·프로필·세션 같은 **허브 공통 기능(`0-x`)은 [planning.md](planning.md)** 에 있다. 이 문서는 한번더 전용만 다룬다.
...
```mermaid
flowchart LR
  H[한번더 홈<br/>방 목록 · 접속자] -->|방 만들기| L[대기실]
  H -->|방 입장| L
  L -->|시작| G[게임]
  G --> R[결과]
  R -->|다시 하기| L
...
본문에 이미 반영했고, 근거를 남기기 위해 표로 둔다. 플랫폼·새로고침처럼 서비스 전체에 걸린 결정은 [planning.md](planning.md)에 있다.
