...
프론트가 다른 출처에서 돌기 때문에 필요하다. **세션 쿠키를 주고받으므로** 아래가 함께 걸린다.

| 항목 | 값 |
| --- | --- |
| `Access-Control-Allow-Credentials` | `true` |
| `Access-Control-Allow-Origin` | **요청한 출처를 그대로 되돌려준다.** `*` 는 쓸 수 없다 |
| 허용 메서드 | `GET` `POST` `PATCH` `PUT` `DELETE` `OPTIONS` |
| 사전 요청 캐시 | 1시간 |

허용할 출처는 설정(`playground.allowed-origins`)에 둔다. **REST 와 소켓 핸드셰이크가 같은 목록을 쓴다** — 프론트가 어디 있는지는 하나의 사실이다.

> 클라이언트는 **요청마다 자격 증명을 켜야 한다.** `fetch` 는 `credentials: 'include'`, `axios` 는 `withCredentials: true` 다. 빠뜨리면 쿠키가 실리지 않아 세션이 매번 새로 만들어진다.
...
> 세션을 보장합니다. 이미 살아 있는 세션이 있으면 그것을 그대로 쓰고, 없을 때만 새로 발급합니다. 로그인이 없으므로 이것이 신원 등록을 대신합니다. (0-7)

**Auth Required:** No (쿠키가 있으면 참고한다)
...
**Response `201`** — 새로 발급했을 때. `Set-Cookie` 가 함께 나간다.

```json
{
  "success": true,
  "data": { "nickname": "두더지4821", "avatar": "⛏" }
}
```

**Response `200`** — 쿠키의 세션이 아직 살아 있을 때. 같은 세션을 그대로 쓰고 **`Set-Cookie` 를 내려주지 않는다.** 보낸 프로필로 이름·아바타만 맞춘다.
...
부를 때마다 새로 만들면 **탭을 하나 더 열 때마다 세션이 늘어난다.** 나중에 발급된 쿠키가 앞의 것을 덮어써서, 한 사람이 접속자 둘로 잡히고 광장 정원에서 두 자리를 먹는다. 먼저 열린 탭은 소켓과 REST 의 신원이 서로 달라진다.

클라이언트가 "나 이미 세션 있나?" 를 스스로 확인할 수도 없다. 쿠키가 `HttpOnly` 라 JS 가 읽지 못한다. 그래서 **서버가 쿠키를 보고 판단한다.** 클라이언트는 진입할 때마다 그냥 호출하면 된다.

#### `sessionKey` 를 응답에 담지 않는다

쿠키를 `HttpOnly` 로 준 이유는 JS 가 읽지 못하게 하려는 것인데, 본문으로 다시 내주면 그 의미가 없다. 클라이언트는 쿠키가 자동으로 붙으므로 값을 알 필요가 없다.
