...
**60초 안에 소켓을 연결하지 않으면 세션이 지워진다.** 이후 요청은 `SESSION_INVALID`로 거절되므로 다시 발급받는다 ([ERD의 세션 TTL 스위퍼](../erd/erd.md#세션-ttl-스위퍼)).
...
어디에 있느냐에 따라 반영 범위가 다르다.

| 있는 곳 | 반영 |
| --- | --- |
| 한번더 방 | **그 방의 표시 이름은 그대로다.** 참여자 이름은 입장 시점 스냅샷이다 ([ERD의 `room_players`](../erd/omt.md#room_players-방-참여자-1-4)) |
| 놀이터 광장 | **즉시 반영된다.** 광장 전원에게 소켓으로 `plaza.profile`이 나간다 ([놀이터 API](playground.md#서버--클라이언트)) |
| 공통 | 접속자 목록(1-13)에는 즉시 반영된다. 이미 보낸 채팅은 바뀌지 않는다 |
...
| 세션 TTL 스위퍼 | 30초마다. 소켓 미연결 60초 · 신호 없음 90초인 세션을 지운다 ([ERD](../erd/erd.md#세션-ttl-스위퍼)) |
| Base URL 도메인 | 아직 미정 |
