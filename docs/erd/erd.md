...
- **세션 TTL 스위퍼**를 둔다. 발급만 받고 소켓을 연결하지 않은 세션, 신호가 끊긴 세션을 지운다 ([SESSIONS](#sessions-접속-중인-사람-1-13))
...
소켓의 끊김 이벤트만 믿으면 정리되지 않는 세션이 생긴다. **30초마다** 스위퍼가 돌며 아래를 지운다.

| 대상 | 조건 | 왜 생기나 |
| --- | --- | --- |
| 소켓을 연결하지 않은 세션 | 발급(`connected_at`) 후 **60초** 안에 소켓이 붙지 않음 | `POST /sessions` 직후 탭을 닫으면 끊김 이벤트가 영영 오지 않는다 |
| 신호가 끊긴 세션 | `last_seen_at` 이 **90초** 이상 지남 (ping 30초 × 3회) | 네트워크가 조용히 끊겨 서버가 close 를 받지 못한 경우 |

지울 때는 소켓 끊김과 **같은 처리**를 한다. 방·광장에 있었다면 거기서 내보낸다 (0-4).
...
`room_players` 와 `plaza_members` 모두 `nickname` · `avatar` · `color_code` 를 **입장 시점에 복사**한다. 같은 이유다. 다만 **프로필을 바꿨을 때의 동작은 카드마다 다르다.**

| 카드 | 프로필을 바꾸면 |
| --- | --- |
| 한번더 (`room_players`) | 그대로다. 입장 시점 값에 고정한다 |
| 놀이터 (`plaza_members`) | **즉시 따라간다.** 서버가 행을 고치고 광장 전원에게 알린다 ([놀이터 ERD](playground.md#이름아바타를-세션과-맞춰-갱신한다-m-p13)) |

어느 쪽이든 **이미 쓴 채팅은 작성 시점 값을 유지한다.**
